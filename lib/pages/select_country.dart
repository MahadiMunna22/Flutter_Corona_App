...
class _SelectCountryState extends State<SelectCountry> {
  ...
  void getCountryName(index) async{
    int i = getCountryIndex(searchItem[index]);
    _write('${searchItem[index]} $i');
    print('CountryName written in txt: ${await _read()}');
    Navigator.pushReplacementNamed(context, '/loading', arguments: {
      'country_name' : searchItem[index],
      'getMapData' : mapCoordinates,
      'country_index': getCountryIndex(searchItem[index]),
    });
    print(getCountryIndex(searchItem[index]));
  }
  int getCountryIndex(String countryName){
    for(int i = 0; country.isNotEmpty ; i++){
      if(countryName == country[i]){
        print('Found Country Index: $i');
        return i;
      }
    }
  }
